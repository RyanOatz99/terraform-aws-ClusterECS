resource "aws_cloudformation_stack" "tf_exports" {
  ...
  template_body = templatefile("${path.module}/cf-exports.yml", {
    ...
    "vars" = {
      ...
      "AlbListenerHttpArn" = length(aws_alb_listener.ecs_http[*].arn) > 0 ? element(aws_alb_listener.ecs_http[*].arn, 0) : "undefined",
      "EcsNodesSecGrpId"   = aws_security_group.ecs_nodes.id,
      "VpcId"              = data.terraform_remote_state.vpc-project-dev.outputs.vpc_id
    }
  })
}