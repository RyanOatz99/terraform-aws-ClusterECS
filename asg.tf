resource "aws_autoscaling_group" "ecs" {
  ...
  vpc_zone_identifier = data.terraform_remote_state.vpc-project-dev.outputs.private_subnet_ids

  min_size = var.asg_min
  max_size = var.asg_max

  tags = [
    map("key", "Name", "value", "ecs-node-${var.name}", "propagate_at_launch", true, "Management", "terraform")
  ]
